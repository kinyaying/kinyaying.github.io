...
hexo.extend.helper.register('get_icon_class', function () {
  let count = Math.floor(Math.random() * 22)
  const iconClassList = [
    '#icon-bellflower',
    '#icon-cornflower',
    '#icon-brokenheartflower',
    '#icon-019-flower',
    '#icon-029-flower',
    '#icon-030-flower',
    '#icon-flower',
  ...
  if (count > 0 && count < 16) {
    return '#icon-flower' + count
  } else {
    return iconClassList[count - 16]
  }
})
